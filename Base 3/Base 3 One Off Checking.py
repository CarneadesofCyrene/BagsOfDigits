...
def run_program_base_3(current_sum): 
    ...
    for a in range(current_sum + 1):
        for b in range(current_sum + 1 - a):
            ...
            zeros3 = base_3_conversion(a)
            ones3 = base_3_conversion(b)
            twos3 = base_3_conversion(c)
            zeros_count = length_of_string(zeros3)
            ones_count = length_of_string(ones3)
            twos_count = length_of_string(twos3)
            total_digits = zeros_count + ones_count + twos_count 
            row = [a, b, c, current_sum, total_digits] 
            all_data.append(row)
...
while FullExit <100000:
    full_data = run_program_base_3(CurrentNumberToCheck) 
    ...
    for node in full_data:
        ...
        BASEcounter_list = [BASEcount0,BASEcount1,BASEcount2] 
        counter_list = [count0,count1,count2] 
        ...
        digit_count = node[3] 
        loseable_digits = collapsed_data + 3 
        ...
        ogcounter_list = [count0,count1,count2] 
        Check_Exit = 0
        history_list = []
        while Check_Exit == 0:
            count0 = tpcount0
            ...
            if tpcount0< 0 or tpcount1<0 or tpcount2<0:
                Check_Exit = 1
            
            else:
                tp_Diagonal = tpcount0 + tpcount1 + tpcount2 
                if tp_Diagonal > last_good_Diagonal:
                    Check_Exit = 1
                    
                
                else:
                    tpcounterlist = [tpcount0,tpcount1,tpcount2]
                    if not(BASEcounter_list in history_list):
                        history_list.append(BASEcounter_list)
                        #print(history_list)
                    if (tpcounterlist == [0,0,0] and Check_Exit==0): 
                        print("newgood")
                        print(history_list)
                        print(Exit)
