...
NoDigits = "[0, 0, 0, 0, 0]"
Location = "C:/Users/thabe/Documents/Carneades/xkcd Puzzle/1.26.26 Cleanup of Existing Data and Gephy/Base 5"
...
for index, row in initial_data.iterrows():
    ...
    while TryCount < MaxColumns:
        ...
        try:
            ...
            MidDF = pd.DataFrame({"Input":row[StartCol],"Output":"[0, 0, 0, 0, 0]"}, index = [0])
            while StartCol >0:
...
final_data.to_csv(Location +"/Cleaned Base 5 324.csv", index = False)                            
...
Collapsed.to_csv(Location + "/Collapsed Base 5 324.csv", index = True) 

###############################################################################
#Renaming

initial_data = pd.read_csv(Location +"/Collapsed Base 5 324.csv")
print(initial_data.head())
...
final_input = pd.DataFrame("We have" + split_input[0] + split_input[1] + split_input[2] + split_input[3] + split_input[4] + ".")
...
final_output = pd.DataFrame("We have" + split_output[0] + split_output[1] + split_output[2] + split_output[3] + split_output[4] +".")
pd.set_option('display.max_columns', None)
...
concatenated_df.to_csv(Location + "/Output_Pairs_Named Base 5 324.csv", index = False)   
