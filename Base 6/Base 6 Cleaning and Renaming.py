...
for index, row in initial_data.iterrows():
    ...
    while TryCount < MaxColumns:
        ...
        try:
            ...
            MidDF = pd.DataFrame({"Input":row[StartCol],"Output":"[0, 0, 0, 0, 0, 0]"}, index = [0])
            while StartCol >0:
...
final_input = pd.DataFrame("We have" + split_input[0] + split_input[1] + split_input[2] + split_input[3] + split_input[4] + split_input[5] + ".")
...
final_output = pd.DataFrame("We have" + split_output[0] + split_output[1] + split_output[2] + split_output[3] + split_output[4] + split_output[5]+".")
pd.set_option('display.max_columns', None)
