...
NoDigits = "[0, 0, 0, 0, 0, 0, 0]"
Location = "BagsOfDigits/Base 7"
...
for index, row in initial_data.iterrows():
    ...
    while TryCount < MaxColumns:
        ...
        try:
            ...
            MidDF = pd.DataFrame({"Input":row[StartCol],"Output":"[0, 0, 0, 0, 0, 0, 0]"}, index = [0])
            while StartCol >0:
...
final_data.to_csv(Location +"/Cleaned Base 7 124.csv", index = False)                            
...
Collapsed.to_csv(Location + "/Collapsed Base 7 124.csv", index = True) 

###############################################################################
#Renaming

initial_data = pd.read_csv(Location +"/Collapsed Base 7 124.csv")
print(initial_data.head())
...
final_input = pd.DataFrame("We have" + split_input[0] + split_input[1] + split_input[2] + split_input[3] + split_input[4] + split_input[5] + split_input[6] + ".")
...
final_output = pd.DataFrame("We have" + split_output[0] + split_output[1] + split_output[2] + split_output[3] + split_output[4] + split_output[5] + split_output[6] +".")
pd.set_option('display.max_columns', None)
...
concatenated_df.to_csv(Location + "/Output_Pairs_Named Base 7 124.csv", index = False)   
